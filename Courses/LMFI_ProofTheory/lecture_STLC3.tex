...
\begin{frame}{Extending with Sums: Normalisation}
	...
	\pause
	\begin{lemma}
		If for all $u'\in \RED_\sigma$ we have $u\{u'/x \}\in \RED_\upsilon$ for all $v'\in \RED_\tau$ we have $v\{v'/y \}\in \RED_\upsilon$, then $\newmatch t x u y v\in \RED_{\upsilon}$.
	\end{lemma}
\end{frame}
...
\begin{frame}{System T: Realisability Interpretation}
	...
	\begin{align*}
		T(n=m)&:=\omega& T(\phi\Rightarrow \psi)&:=T(\phi)\to T(\psi)\\
		T(\phi\land \psi)&:=T(\phi)\times T(\psi)&T(\forall x A)&:= \omega \to T(A)\\
		T(\phi\lor \psi)&:=T(\phi)+T(\psi)&T(\exists x A)&:= \omega \times T(A)
	\end{align*}
	
	\pause
	Moreover, we can interpreters as their sets of realising terms of System T:
	\begin{align*}
		t\Vdash n=m &~:=~n \text{ equals } m& t\Vdash \phi\Rightarrow \psi&~:=~ tu\Vdash \psi \text{ for all } u\Vdash \phi\\
		t\Vdash \phi\land \psi &~:=~\pi_1t \Vdash \phi\text{ and } \pi_2t \Vdash \psi& t\Vdash \forall x A &~:=~ t\overline n\Vdash A[\overline n/x] \text{ for all }n\in \mathbb{N} \\
		\iota_i t\Vdash \phi_1\lor \phi_2 &~:=~t\Vdash \phi_i& t\Vdash \exists x A&:= \pi_2 t\Vdash A[\pi_1 t/x]
	\end{align*}
	
	\pause
	\begin{theorem}
		If $HA\vdash \phi$ then there is $t:T(\phi)$ such that $t\Vdash \phi$.
	\end{theorem}
\end{frame}
