...
\begin{frame}{Extending with Sums: Normalisation}
	\begin{enumerate}
		...
		\pause
		\item
		Define $\RED_{\sigma+\tau}:= \{\Gamma \vdash t : \sigma+\tau \mid t\longrightarrow_\beta^* \iota_1 t'\in \RED_\sigma \lor t\longrightarrow_\beta^* \iota_2 t'\in \RED_\tau \}$.
		\vspace{0.3cm}
		\pause
		\item
	...
	\pause
	\begin{lemma}
		If for all $u'\in \RED_\sigma$ we have $u\{u'/x \}\in \RED_\rho$ for all $v'\in \RED_\tau$ we have $v\{v'/y \}\in \RED_\rho$, then $\newmatch t x u y v\in \RED_{\rho}$.
	\end{lemma}
\end{frame}
