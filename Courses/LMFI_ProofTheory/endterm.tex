...
\noindent
This part represents half of the points of the mid-term. In my opinion, it is reasonable to spend around 20min on Qeustion 1, 30min on Question 2 and 40min on Question 4.
...
\begin{exercise}[F-Translation]
	...
	\begin{enumerate}
		...
		\item
		Derive that $\Gamma\vdash A$ implies $\Gamma^F\vdash A^F$. It suffices to show that $\Gamma^F\vdash \bot^F$ implies $\Gamma^F\vdash A^F$.
		\item
		Justify briefly why $H\!A \vdash H\!A^F$, i.e.~HA proves every $F$-translated axiom.
		\item
		Assuming that $H\!A\vdash \neg\neg \exists x.t(x)=0$, choose $F$ suitable and derive that $H\!A\vdash\exists x.t(x)=0$.
	\end{enumerate}
\end{exercise}
...
\begin{exercise}[Strong Normalisation]
	We extend STLC with a unit type $\top$ and a void type $\bot$
	$$\infer[TI]{\Gamma\vdash *:\top}{}\hspace{3em}
	\infer[BE]{\Gamma\vdash E\,t:\sigma}{\Gamma\vdash t:\bot}
	$$
	including the additional congruence reduction $E\,t\longrightarrow_\beta E\,t'$ whenever $t\longrightarrow_\beta t'$.
	\begin{enumerate}
		\item
		For $\sigma\in\{\bot,\top\}$ set $\RED_\sigma:=\{  \Gamma\vdash t:\sigma \mid t\in \SN \}$ and establish:
		\begin{enumerate}
			\item[P1:]
		...
		\end{enumerate}
		\item
		Extend the inductive proof that $\Gamma\vdash t:\sigma$ implies $t\{\rho\}\in \RED_\sigma$ to (TI) and (BE).
		Recall that here $\{\rho\}=\{u_1/x_1,\dots,u_k/x_k\}$ substitutes all free variables $x_i$ of $t$ by $u_i\in \RED_{\sigma_i}$ where $(x_i:\sigma_i)\in \Gamma$.
		\item
		Derive that there is no closed term $t:\bot$. What does this mean for (propositional) NJ?
	\end{enumerate}
\end{exercise}
...
NJ is the system $\Gamma\vdash A$ and NK is the system $\Gamma\vdash_c A$ with the additional rule (DN).
